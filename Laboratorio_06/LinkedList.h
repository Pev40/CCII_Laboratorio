...
/***SOBRECARGAS***/
//Sobrecarga de operador copia
template<typename G>
LinkedList<G> &LinkedList<G>::operator=(const LinkedList<G> &o){
    this->longitud=0;
    this->head=nullptr;
    Node<G>*iterador=o.head;
    while (iterador!=nullptr){
...
template<typename G>
LinkedList<G>& LinkedList<G>:: operator=(LinkedList<G> &&o){
    this->longitud=0;
    this->head=nullptr;
    Node<G>*iterador=o.head;
    while (iterador!=nullptr){
...
template<typename G>
void LinkedList<G>::deleteToPosition(int position){
    ...
    if(position<this->longitud){
        ...
        std::cout<<"No es posible borrarlo, no existe la posición"<<std::endl;
        }
}
